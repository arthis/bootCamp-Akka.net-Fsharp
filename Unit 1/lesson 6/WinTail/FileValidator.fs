module FileValidator
    ...
    let actor (consoleWriter:ActorRef)  =
        ...
        fun (mailbox:Actor<FileValidator>) (msg:FileValidator) -> 
            ...
            let react = fun m ->
                ...
                match m with
                | Processing(f,m) ->
                    let tailCoordinatorActor = mailbox.Context.ActorSelection("/user/tailCoordinatorActor")
                    tailCoordinatorActor.Tell(StartTail(f,consoleWriter))
                | NullError(r) | ValidationError(r)  | ProcessingError(r) -> mailbox.Sender().Tell(ContinueProcessing)
